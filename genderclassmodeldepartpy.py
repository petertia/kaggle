...
csv_file_object = csv.reader(open('C:/Users/Peter/Documents/Kaggle/Titanic/train2.csv','rt'))
...
number_of_embark_dest = 3
#Define survival table
survival_table = np.zeros([2, number_of_classes, number_of_price_brackets, number_of_embark_dest])

for i in range(number_of_classes):
        for j in range(number_of_price_brackets):
                for k in range(number_of_embark_dest):
...
with open('C:/Users/Peter/Documents/Kaggle/Titanic/test2.csv', newline='') as csvTestfile:
        test_file_object = csv.reader(csvTestfile, dialect='excel')
        header = next(test_file_object)
        with open('C:/Users/Peter/Documents/Kaggle/Titanic/genderclassmodeldepartpy.csv', 'w', newline='') as csvWriterfile:
                ...
                for row in test_file_object: #loop through each passenger in the test file
                        ...
                        if row[4] == 'female':
                                row.insert(1, int(survival_table[0,float(row[1])-1, bin_fare,embark]))
                                open_file_object.writerow((row[0],row[1]))
                                print(int(survival_table[0,float(row[1])-1, bin_fare,embark]))
                        else:
                                row.insert(1, int(survival_table[1,float(row[1])-1,bin_fare,embark]))
                                open_file_object.writerow((row[0],row[1]))
                                print(int(survival_table[0,float(row[1])-1, bin_fare,embark]))
